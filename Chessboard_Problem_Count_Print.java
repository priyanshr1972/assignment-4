...
//
//        a. Write a recursive function which returns the val of different distinct ways this board can be crossed. Print the value returned.
//
...
public class Chessboard_Problem_val_Print {
    ...
    public static int fPath(int n, String res, int rowAtEnd, int col_end, int currentRow, int currentCol) {
        ...
        int val=0;
        val+= fPath(n, res + "{" + currentRow + "-" + currentCol + "}K", rowAtEnd, col_end, currentRow + 2, currentCol + 1);
        val+= fPath(n, res + "{" + currentRow + "-" + currentCol + "}K", rowAtEnd, col_end, currentRow + 1, currentCol + 2);
        if (currentCol == 0 || currentRow == 0 || currentCol == n - 1 || currentRow == n - 1) {
            for (int i = 1; i <= rowAtEnd; i++) {
                val += fPath(n, res + "{" + currentRow + "-" + currentCol + "}R", rowAtEnd, col_end, currentRow, currentCol + i);
            }
            for (int i = 1; i <= rowAtEnd; i++) {
                val += fPath(n, res + "{" + currentRow + "-" + currentCol + "}R", rowAtEnd, col_end, currentRow + i, currentCol);
            }
        }
        if (currentCol == currentRow || currentCol + currentRow == rowAtEnd) {
            for (int i = 1; i <= rowAtEnd; i++) {
                val+= fPath(n, res + "{" + currentRow + "-" + currentCol + "}B", rowAtEnd, col_end, currentRow + i, currentCol + i);
